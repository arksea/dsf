...
namespace com.google.common.util.concurrent
{
	...
	internal abstract class SmoothRateLimiter : RateLimiter
	{
	  ...
	  {
		internal readonly long warmupPeriodMicros;
		/// <summary>
		/// The slope of the line from the stable interval (when permits == 0), to the cold interval
		/// (when permits == maxPermits)
		/// </summary>
		internal double slope;
		internal double thresholdPermits;
		internal double coldFactor;

		internal SmoothWarmingUp(SleepingStopwatch stopwatch, long warmupPeriodMicro, double coldFactor) : base(stopwatch)
		{
          this.warmupPeriodMicros = warmupPeriodMicro;
		  this.coldFactor = coldFactor;
		}

		internal override void doSetRate(double permitsPerSecond, double stableIntervalMicros)
		{
		  ...
		  {
			storedPermits = (oldMaxPermits == 0.0) ? maxPermits : storedPermits * maxPermits / oldMaxPermits; // initial state is cold
		  }
		}

		internal override long storedPermitsToWaitTime(double storedPermits, double permitsToTake)
		{
		  ...
		  {
			double permitsAboveThresholdToTake = System.Math.Min(availablePermitsAboveThreshold, permitsToTake);
		...
		internal double permitsToTime(double permits)
		{
	  ...
	  {
		/// <summary>
		/// The work (permits) of how many seconds can be saved up if this RateLimiter is unused? </summary>
		internal readonly double maxBurstSeconds;

		internal SmoothBursty(SleepingStopwatch stopwatch, double maxBurstSeconds) : base(stopwatch)
		{
		...
		internal override void doSetRate(double permitsPerSecond, double stableIntervalMicros)
		{
		  ...
		  {
			storedPermits = (oldMaxPermits == 0.0) ? 0.0 : storedPermits * maxPermits / oldMaxPermits; // initial state
		  }
		}
	  ...
	  private SmoothRateLimiter(SleepingStopwatch stopwatch) : base(stopwatch)
	  {
	  }

	  internal override sealed void doSetRate(double permitsPerSecond, long nowMicros)
	  {
		resync(nowMicros);
		double stableIntervalMicros = 1000000L / permitsPerSecond;
		this.stableIntervalMicros = stableIntervalMicros;
		doSetRate(permitsPerSecond, stableIntervalMicros);
	  }

	  internal abstract void doSetRate(double permitsPerSecond, double stableIntervalMicros);

	  internal override sealed double doGetRate()
	  {
          return 1000000L / stableIntervalMicros;
	  }

	  internal override sealed long queryEarliestAvailable(long nowMicros)
	  {
		return nextFreeTicketMicros;
	  }

	  internal override sealed long reserveEarliestAvailable(int requiredPermits, long nowMicros)
	  {
		resync(nowMicros);
		long returnValue = nextFreeTicketMicros;
		double storedPermitsToSpend = System.Math.Min(requiredPermits, this.storedPermits);
	  ...
	  /// 
	  /// <para>This always holds: {@code 0 <= permitsToTake <= storedPermits}
	  /// </para>
	  /// </summary>
	  internal abstract long storedPermitsToWaitTime(double storedPermits, double permitsToTake);
	  ...
	  private void resync(long nowMicros)
	  {
		...
		{
		  storedPermits = System.Math.Min(maxPermits, storedPermits + (nowMicros - nextFreeTicketMicros) / coolDownIntervalMicros());

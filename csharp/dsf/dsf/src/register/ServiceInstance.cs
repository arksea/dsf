...
namespace com.baidu.dsf.register
{
    ...
    using RateLimiter = com.google.common.util.concurrent.RateLimiter;
    ...
    {
        private const long DEF_RATE_UPGRADE_PERIOD = 15000;
        private const double DEF_RATE_UPGRADE = 1.5;
        private const int MULTIPLE = 10; //为了在Step很小时做流量提升更加平滑
                                                //permitsPerSecond与step在实际使用时
                                                //乘以此数进行放大
        protected internal readonly MsgSvcAddr addr;
        private MsgServiceDefine svcdef;
        protected internal readonly ConcurrentHashMap states = new ConcurrentHashMap();
        ...
        public virtual void init(MsgServiceDefine def)
        {
            lock (this)
            {
                this.svcdef = def;
                ...
                minRateLimit = 1;
                enableClientRateLimitMaxRT = int.MaxValue;
                ...
                requestCount = 0;
                lastRequestCount = 0;
                lastQPSTime = java.lang.System.currentTimeMillis();
                QPS = 0;
        ...
        internal virtual void setState(string name, object value)
        {
        ...
        //更新限流参数
        private void resetRateLimiter(object value)
        {
            lock (this)
            {
                try
                {
                    ...
                    if (vl.Length == 3)
                    {
                        ...
                        permits = Convert.ToInt32(vl[1]);
                        min = Convert.ToInt32(vl[2]);
                    }
                    else
                    {
                        enableClientRateLimit = false;
                ...
                {
                    DSFLogHelper.ErrorLog("ServiceInstance", ex, "更新限流参数失败");
                }
        ...
        //更新限流参数
        private void resetRateLimiterRT(object value)
        {
            lock (this)
            {
                try
                {
                    enableClientRateLimitMaxRT = Convert.ToInt32(value.ToString());
                }
                catch (Exception ex)
                {
                    DSFLogHelper.ErrorLog("ServiceInstance", ex, "更新开启限流RT门限失败");
                }
            }
        }

        //更新限流提升策略参数
        private void resetRateUpgrade(object value)
        {
            lock (this)
            {
                try
                {
                    ...
                    if (vl.Length == 2)
                    {
                        n = Convert.ToDouble(vl[0]);
                        period = Convert.ToInt64(vl[1]);
                    }
                    else if (vl.Length == 1)
                    {
                        n = Convert.ToDouble(vl[0]);
                ...
                {
                    DSFLogHelper.ErrorLog("ServiceInstance", ex, "更新限流提升策略参数失败");
                }
            }
        }

        public virtual void onServiceFailed(Exception ex)
        {
            //在出现超时错误时启用超时流控策略
            bool exIs = ex is java.net.SocketTimeoutException || ex is TimeoutException;
            Exception cause = ex.InnerException;
            bool causeIs = cause!=null && (cause is java.net.SocketTimeoutException || cause is TimeoutException);
            if (exIs || causeIs)
            {
                lock (this)
                {
                    if (timeoutCount == 0)
                    {
                        ...
                        lastChangeStepTime = java.lang.System.currentTimeMillis();
                    }
        ...
        public virtual void onServiceSucceed(long rt)
        {
            lock (this)
            {
        ...
        public virtual bool tryAcquire()
        {
            lock (this)
            {
                ...
                else
                {
                    ...
                    //permitsPerSecond与step在实际使用时乘以10的目的是
                    //为了在Step很小时做流量提升更加平滑
                    bool success = rateLimiter.tryAcquire(acquireStep);
                    if (success)
                    {
                        requestSuccess();
                    }
            ...
            if (t > 5)
            {
                ...
                if (enableClientRateLimit)
                {
                    ...
                    if (r > 70)
                    {
                        ...
                        if (r > 80)
                        {
                            DSFLogHelper.ErrorLog("ServiceInstance", msg);
                        }
                        else
                        {
                            DSFLogHelper.WarnLog("ServiceInstance", msg);
                        }
                    }
                }
            }
        ...
        public virtual void onConnectSucceed()
        {
            states.put("online", "true");
        }

        public virtual void onConnectFailed(Exception exception)
        {
            //因为网络原因造成的错误将暂时把服务实例设置为离线状态，直到服务器主动将其设置为在线
        ...
        private void resetAcquireStep(int step, string logMsg)
        {
            ...
            {
                //logger.warn(logMsg+": Step = "+step);
                DSFLogHelper.WarnLog("ServiceInstance",logMsg + ": Step = " + step + ", failedCount=" + timeoutCount + ", QPS=" + QPS);
                lastChangeStepTime = java.lang.System.currentTimeMillis();
                this.acquireStep = step;
            }
        }

        public virtual MsgSvcAddr Addr
        {
            get
            {
                return addr;
        ...
        public virtual object getState(string name)
        {

...
namespace DynamicProxy
{
	...
	public class ProxyFactory
	{
        ...
        public Object Create(IProxyInvocationHandler handler, Type objType, bool isObjInterface, Type[] appendInterfaces)
        {
            string typeName = objType.FullName + PROXY_SUFFIX;
            Type type = (Type)typeMap[typeName];
            ...
            if (type == null)
            {
                lock (typeMap)
                {
                    ...
                    if (type == null)
                    {
                        if (isObjInterface)
                        {
                            System.Type[] newifs = new System.Type[appendInterfaces.Length + 1];
                            System.Array.Copy(appendInterfaces, newifs, appendInterfaces.Length);
                            newifs[appendInterfaces.Length] = objType;
                            type = CreateType(handler, newifs, typeName);
                        }
                        else
                        {
                            System.Type[] ifs = objType.GetInterfaces();
                            System.Type[] newifs = new System.Type[ifs.Length + appendInterfaces.Length];
                            System.Array.Copy(ifs, newifs, ifs.Length);
                            System.Array.Copy(appendInterfaces, 0, newifs, ifs.Length, appendInterfaces.Length);
                            type = CreateType(handler, newifs, typeName);
                        }
                        typeMap.Add(typeName, type);
                    }

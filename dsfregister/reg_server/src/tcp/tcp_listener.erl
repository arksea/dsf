...
start_link(IPAddress, Port, SocketOpts,
    ...
    Name = list_to_atom(lists:flatten(io_lib:format("~s_listener", [Label]))),
...
init({IPAddress, Port, SocketOpts,
    ...
    case gen_tcp:listen(Port, SocketOpts ++ [{ip, IPAddress},
        ...
        {ok, LSock} ->
            ...
            ?LOG_INFO("tcp listener started",[Label, inet_parse:ntoa(LIPAddress), LPort]),
            apply(M, F, A ++ [IPAddress, Port]),
            {ok, #state{sock = LSock,
                        on_startup = OnStartup, on_shutdown = OnShutdown, 
                        label = Label}};
        {error, Reason} ->
            ?LOG_INFO("failed to start tcp listener",[Label, inet_parse:ntoa(IPAddress), Port, Reason]),
...
terminate(_Reason, #state{sock=LSock, on_shutdown = {M,F,A}, label=Label}) ->
    ...
    gen_tcp:close(LSock),
    ?LOG_INFO("tcp listener stopped",[Label, inet_parse:ntoa(IPAddress), Port]),
    apply(M, F, A ++ [IPAddress, Port]).

...
-define(DEF_TABLE(RecordName),
    {RecordName,[{record_name, RecordName},
                 {attributes,  record_info(fields, RecordName)},
                 {disc_copies, [node()]}]}).
-define(DEF_TYPE_TABLE(RecordName,Type),
       {RecordName,[{type, Type},
...
table_definitions() ->
    [
    ?DEF_TABLE(dsconfig),               %静态配置表
    ?DEF_TABLE(svcdef),                 %应用服务定义表
    ?DEF_TABLE(svcdef_idl),             %应用服务接口定义
    ?DEF_TYPE_TABLE(subscribe_map, bag), %服务订阅者映射表
    ?DEF_TABLE(dsfclient),               %客户端状态表
    ?DEF_TABLE(dservice_state)          %应用服务状态表
    ].
...
cluster_nodes_config_filename() ->
    {ok, Vsn} = application:get_key(reg_server, vsn),
    "releases/" ++ Vsn ++ "/mnesia.config".
                    ...
                    case file:consult(DefaultFileName) of
                        ...
                        {error, enoent} ->
                            error_logger:warning_msg(
                              "default db cluster config file ~p does not exist~n",
                              [DefaultFileName]),
                            [];
                        {error, Reason} ->

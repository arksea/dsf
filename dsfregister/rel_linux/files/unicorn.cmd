...
@if "%1"=="console_clean" @goto console_clean
@if "%1"=="ping" @goto ping
...
:usage
@echo Usage: %~n0 [install^|uninstall^|start^|stop^|restart^|console^|console_clean^|ping^|query^|attach^|upgrade]
@goto :EOF
...
:console_clean
@start "%node_name% console" %werl% -boot "%clean_boot_script%" -config "%sys_config%" -args_file "%vm_args%" -sname %node_name%
@goto :EOF
